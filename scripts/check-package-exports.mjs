...
//
// Why: a subpath that a declaration names but `package.json#exports` lacks is rejected by Node
// and every bundler (ERR_PACKAGE_PATH_NOT_EXPORTED), and a `files` glob that misses an extension
// (`*.d.ts` vs `*.d.mts`) leaves the declaration out of the tarball. Both are invisible in the
// repo (everything resolves fine from source) and only surface in a consumer's install. So the
// check runs against the REAL tarball contents (`npm pack --dry-run`), not against the working tree.
//
