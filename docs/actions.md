...
`profileWorker(seconds?, dir?, thread?) → { thread, seconds, file, facadeCount, sampledMs, idlePercent, gcPercent, wasmPercent, topFunctions[], topFiles[], topInclusive[], heapBefore, heapAfter, gc }` (0.21.4, `thread` + heap/gc 0.21.5) - CPU profile of the wallet worker thread (`thread: 'worker'`, default) or of the CAP process's main thread (`'main'`: request handling, the state-save pipeline, pollers), taken with the in-thread V8 profiler for `seconds` (1..120, default 20) while the thread keeps serving. The summary says where the time went (self time by function and by file, inclusive hot paths, each as a share of the sampled window, plus the idle/GC/wasm shares), `heapBefore`/`heapAfter` give used/total/limit/external heap, malloced, RSS and ArrayBuffer MB of that isolate, `gc` counts the collections in the window with total ms and a JSON `byKind` (minor/major/incremental). The raw `.cpuprofile` is written under `dir` (default: OS temp dir, `nightgate-profiles/`) for Chrome DevTools. The caller waits for the window (request timeout `seconds + 60 s`). Use it when the process is busy and the log does not say why; reference: a warm facade at tip idles above 90 % of the worker, GC under 10 %.
