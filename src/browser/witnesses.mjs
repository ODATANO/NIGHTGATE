...
//
// This is the ONE implementation of the vault witness builder and the
// attester-secret derivation: the server (`srv/submission/contract-witnesses.ts`)
// re-exports it through Node's require(esm), so the browser bundle, the
// txbuilder and the worker thread decode proof bundles with identical code.
// Kept dependency-clean (no Node built-ins, no `srv/`).
...
import { hexToBytes, hexToBytes32, bytesToHex } from './hex.mjs';
