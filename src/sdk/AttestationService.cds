 ...
 *
 * Abstract CAP service that consumer apps (e.g. @odatano/passport) can extend to
 ...
 * `registerAttestationServiceHandlers` TS helper and it reads
 * `req.disclosureRole` set by the `attachDisclosureRole` middleware
 *
 * Consumer pattern:
 *
