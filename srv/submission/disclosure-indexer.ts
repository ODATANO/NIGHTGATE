/**
 * Disclosure-grant chain indexer. Reads the AttestationVault `disclosures`
 * ledger Map from on-chain state into the `DisclosureGrants` entity, so tier
 * entitlement is a queryable, chain-derived source of truth (the contract is the
 * ACL).
 *
 * CENTRAL CONSTRAINT (validated in scripts/spike-disclosure-indexer.mjs): the
 * OUTER `disclosures` map is NOT iterable (only `member`/`lookup`); the inner
 * per-payload map and the sibling `attestation_owners` map ARE. So we enumerate
 * payload hashes via `attestation_owners` (every disclosure is scoped to an
 * attested payload), then drill into `disclosures.lookup(payloadHash)`.
 *
 * Decode + enumerate logic is dependency-injected (`ledger`, `queryContractState`)
 * to unit-test without the ESM-only SDK; `reindexDisclosuresForContract` wires the
...
export async function reindexDisclosures(deps: ReindexDeps): Promise<ReindexResult> {
    ...
    // Sweep: an active row no longer on-chain was revoked; flip it inactive. Skip
    // rows touched within the grace window (queried state may not yet include a
    // just-submitted grant, and sweeping it would deactivate a live grant).
