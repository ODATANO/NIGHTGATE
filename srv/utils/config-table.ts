...
export const CONFIG_TABLE: readonly ConfigSpec[] = [
    ...
    { key: 'NIGHTGATE_ACCEPT_UNBOUND_ENVELOPES', kind: 'bool', default: false, doc: '`true` still reads v1/v2 ciphertexts in columns bound to a purpose and a row (v3); only until `nightgate-rewrap-keys` rewrote them' },
    { key: 'NIGHTGATE_ALLOW_UNAUTHENTICATED', kind: 'bool', default: false, doc: '`true` permits `auth.kind: dummy` in production (every request unauthenticated and privileged); local testing only' },
    ...
    { key: 'NIGHTGATE_DUST_RACE_RETRIES', kind: 'int', default: 2, min: 0, doc: "Rebuild-retries of a bound deploy/call/batch on a transient dust race (`1010/170`, `1010/171`, `1010/196`, pre-mempool, fee unspent); default `2`. Each retry re-proves the call, hence smaller than the sponsor path's `NIGHTGATE_SPONSOR_DUST_RETRIES`. `0` disables." },
